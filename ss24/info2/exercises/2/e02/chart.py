...
def plot_gantt_chart_sjf(df):
    ...
    ax.set_xlabel("t")
    ax.set_ylabel("Prozess")
    ax.set_title("SJF Lauf- und Wartezeiten")

    # plt.show()
    plt.savefig("build/sjf_gantt.png")
...
if __name__ == "__main__":
    ...
    sjf_states, sjf_completed = sjf_scheduling(processes)
    # plot_gantt_chart_sjf(sjf_completed)
    sjf_states.columns = [f"${c}$" for c in sjf_states.columns]
    sjf_completed.columns = [f"${c}$" for c in sjf_completed.columns]
    print("Shortest Job First Serve Scheduling")
    # print_markdown_table(fcfs_states)
    print_markdown_table(sjf_completed)
    print(f"Mean waiting time: {sjf_completed['$w$'].mean()}")
    sjf_states.to_markdown(buf="build/sjf_steps.md")
    sjf_completed.to_markdown(buf="build/sjf_result.md")
